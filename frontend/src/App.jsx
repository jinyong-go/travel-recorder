...
import './App.css'
import { CATEGORIES, MOCK_PLACES, categoryIcon, categoryLabel } from './data/places.js'
...
function App() {
  ...
  const filtered = useMemo(() => {
    const byCategory =
      category === 'all' ? MOCK_PLACES : MOCK_PLACES.filter((p) => p.category === category)
      ...
      {registerOpen && (
        <div className="modal-overlay" onClick={() => setRegisterOpen(false)}>
          ...
          >
            ...
            <h2>여행지 등록</h2>
            <p className="modal-desc">등록 화면은 준비 중입니다. 곧 만나요!</p>
          </div>
        </div>
      )}
