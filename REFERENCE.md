...
받은 초대와 보낸 초대를 한 화면에서 보고 **지난 초대까지 조회**하고 싶다는 요구에서 출발. 기존 모델은 수락·거절·철회가 모두 `group_invite` 행 삭제로 끝나 **끝난 초대가 어디에도 남지 않았다.**

**먼저 확인한 것**

- "수락된 초대"의 결과는 이미 조회된다 — `group_member.joined_at` 이 누가 언제 들어왔는지를 갖고 있다. 없는 것은 **거절·취소된 초대의 흔적**뿐이다.
- 흔적을 남기지 않는 것은 실수가 아니라 이전 판의 명시적 결정이었다 — 거절이 보낸 사람에게 드러나지 않아야 사회적 마찰이 없고, 그래서 "거절한 사람을 다시 초대할 수 있다"가 성립했다.

**선택지와 판단**

| | 내용 | 채택 |
|---|---|:---:|
| 대기 중인 초대만 한 화면에 모은다 | DB 변경 없음. "이력"이라는 이름만 포기 | ❌ |
| 끝난 초대를 남긴다 | 도메인 규칙 개정이 선행 | ✅ |

이력을 남기기로 한 이상 **거절을 감추지 않기로** 했다. 감춰도 보낸 목록에서 항목이 사라진 것으로 추론되고, 절반만 가린 이력은 읽는 사람이 사실과 다르게 이해하게 만든다. 대신 거절 확인 모달에서 **"거절한 사실이 보낸 사람에게도 보입니다"** 를 누르기 전에 알린다.

**저장 방식** — `group_invite` 에 상태 컬럼을 달지 않고 **별도 `invite_history` 테이블**로 옮긴다.

| | 상태 컬럼 + soft delete | 별도 이력 테이블 |
|---|---|---|
| `unique(group_id, invitee_id)` | 끝난 초대가 자리를 차지해 **재초대가 막힌다.** 부분 유니크 인덱스로 우회해야 함 | 그대로 살아 있다 |
| H2(local) ↔ PostgreSQL | 부분 인덱스 문법이 갈린다. 마이그레이션 도구도 없다 | 영향 없음 |
| 대기 초대 조회 | 모든 쿼리에 상태 조건이 붙는다 | 경로가 그대로다 |

**그룹 삭제 대응** — 그룹이 지워져도 이력은 남아야 하므로 `group_id` 에 **외래키를 걸지 않고 `group_name` 스냅샷**을 함께 저장한다. 장소 정보를 기록에 복사해 두는 것(공통 명세 §3.8)과 같은 이유다. 대기 중이던 초대는 `GROUP_DELETED` 로 이력에 남는다 — 받은 쪽에서는 초대가 이유 없이 사라지는 일이기 때문이다.

> 이 검토는 **유사 서비스 조사 없이** 기존 모델의 제약과 본 프로젝트의 정책만으로 결론냈다. 초대 이력을 외부에 드러내는 방식은 서비스마다 다를 수 있어, 거절 공개가 실제 사용에서 문제가 되면 재검토 대상이다.

## 미결 사항

- **사용자 탈퇴 시 초대 이력 처리.** 이력은 지우지 않기로 했으나(공통 명세 §3.7) 탈퇴한 사용자의 이름·프로필이 상대의 이력에 남는 문제는 탈퇴 기능 자체와 함께 범위 밖이다 (공통 명세 §7.3).

검토 2의 "공동 편집 여부"는 **조회 전용 재사용 리스트**로 결론났고(편집 권한은 작성자 본인만), 검토 3에서 공유의 단위가 여행임을 확정했다.
